 ...
 * class if you want to create a GWT RPC DataSource for SmartGWT. This is based on the {@link GwtRpcDataSource} example
 * provided in the smartgwt-extensions.
 * 
 * In order to use this class, you have to implement both {@link GenericGwtRpcService} and
 * {@link GenericGwtRpcServiceAsync} provided in the same package. To use paging ({@link FetchMode#PAGED}), you'll have
 * to return a {@link GenericGwtRpcList} from the fetch()-method of your {@link GenericGwtRpcService}-implementation.
 * 
 * @param <D>
 *            type of the transfer object holding the data (will most likely be a simple POJO), must implement
 *            {@link Serializable} or {@link IsSerializable}.
 * @param <R>
 *            any extension of {@link Record}, such as {@link ListGridRecord}, {@link DetailViewerRecord} or
 *            {@link TreeNode} to use with your SmartGWT widget.
 * @param <SA>
 *            the asynchronous version of your service. Extend {@link GenericGwtRpcService} and then
...
public abstract class GenericGwtRpcDataSource<D, R extends Record, SA extends GenericGwtRpcServiceAsync<D>>
	...
	/**
	 * @return a list of {@link DataSourceField}, used to define the fields of your {@link DataSource}. NOTE: Make sure
	 *         to set a primary key, as some problems might occur if it's omitted.
	 */
	 ...
	 * 
	 *         This is unfortunately necessary as <code>GWT.create()</code> only allows class literal as argument. We
	 *         cannot create a class literal from a parameterized type because it has no exact runtime type
	 *         representation, which is due to type erasure at compile time.
	 */
	public abstract SA getServiceAsync();

	/**
	 * @return a new instance of your {@link Record}, such as <code>new Record()</code> or
	 *         <code>new ListGridRecord()</code>.
	 * 
	 *         This method is needed because we cannot instantiate parameterized types at runtime. It also increases
	 *         flexibility as we can pass more complex default objects.
	 */
	public abstract R getNewRecordInstance();

	/**
	 * @return a new instance of your data object, such as <code>new YourDataObject()</code>.
	 * 
	 *         This method is needed because we cannot instantiate parameterized types at runtime. It also increases
	 *         flexibility as we can pass more complex default objects.
	 */
